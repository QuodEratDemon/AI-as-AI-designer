...
#AG: Declaring base mod list. 0: danger_curve, 1,2,3: bomb_ratio_curve, 4: smart_to_bomb_curve, 5,6: time_impact_curve
mod = [1.0001,.3,.2,.3,.5,4,.25]
...
#      self.move.put((escape_priority,chosen_movement_action))
    ...
    #NH:decisions
    if chosen_movement_action != None or self.move._qsize() < 1:
...
#        print(i)
        if escape_direction_ratings[i] != best_rating:
          best_actions[i]
            
        
      chosen_movement_action = random.choice(best_actions)
