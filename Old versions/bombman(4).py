...
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        ...
        if len(spot.player) > 1:
          ...
          if SameAsPlayer is False:
            ...
            break
        if new not in visited and self.distance_formula(start,new) <= 3 and new[0] <= 10 and new[1] <= 14:
#          print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
  ...
  def get_tile_neighbors2(self, tile_coordinates, came_from):
    ...
    if self.game_map.tile_has_teleport:
      teleports_to = self.game_map.get_tile_at(tile_coordinates).destination_teleport
...
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        ...
        if new is None:
          continue
        if not self.game_map.tile_has_teleport or not self.game_map.tile_is_walkable:
          continue
...
#          allies += 1
  ...
  def play(self):
    ...
    if direction is not None:
      ...
      self.move.put((-999999999 ,chosen_movement_action))
