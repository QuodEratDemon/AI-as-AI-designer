...
#AG: Declaring base mod list. 0: danger_curve
mod = [1.0001]
...
#        time_until_explosion = 100
      ...
      for i in range(Map.MAP_HEIGHT):  # reset
        ...
        for j in range(Map.MAP_WIDTH):
          str_danger = str(self.danger_map[i][j])
          f.write(str_danger)
          f.write(" ")
      f.write('\n')
...
#          allies += 1
  ...
  def play(self):
    ...
    #AG: Utility curve function
    def ratio_bomb_curve(ratio):
      #AG: base curve is y=(1/(1+e^(7*(x-.8))))-.1
#      denom = 1+pow(math.e,(7*(ratio-.8)))
#      y = 1/denom-.1
#      return y
      #y=(1/(1+e^(9*(1-.4))))+.2
      denom = 1.3+pow(math.e,(7.5*(ratio-.4)))
      y = 1/denom+.2
      return y
    '''
