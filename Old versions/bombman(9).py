...
#AG: Declaring base mod list. 0: danger_curve, 1,2,3: bomb_ratio_curve, 4: smart_to_bomb_curve, 5,6: time_impact_curve,7:bomb_throw_curve
mod = [1.0001,.3,.2,.3,.5,4,.45,-10]
...
n = open("smart_to_bomb_curve.txt",'w')
...
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        if new not in visited and self.distance_formula(start,new) <= 7 and new[0] <= 10 and new[1] <= 14:
#          print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
  ...
  def get_tile_neighbors2(self, tile_coordinates, came_from):
    ...
    if self.game_map.tile_has_teleport:
      teleports_to = self.game_map.tiles[tile_coordinates[0]][tile_coordinates[1]].destination_teleport
      if came_from[tile_coordinates] is not teleports_to:
        neighbors.append(teleports_to)
        return neighbors
      
    if self.game_map.tile_is_withing_map((x+1,y)) and self.game_map.tile_is_walkable((x+1,y)):
      neighbors.append((x+1,y))
    if self.game_map.tile_is_withing_map((x-1,y)) and self.game_map.tile_is_walkable((x-1,y)):
      neighbors.append((x-1,y))
    if self.game_map.tile_is_withing_map((x,y+1)) and self.game_map.tile_is_walkable((x,y+1)):
      neighbors.append((x,y+1))
    if self.game_map.tile_is_withing_map((x,y-1)) and self.game_map.tile_is_walkable((x,y-1)):
      neighbors.append((x,y-1))
    return neighbors
  ...
  def path_to_target(self, destination):
    ...
    while len(frontier) > 0:
      current = heappop(frontier)
      print("if "+str(current[1])+" is "+str(destination))
      if current[1] == destination:
        pathFound = True
        break
    
      if not self.game_map.tile_has_teleport or not self.game_map.tile_is_walkable(current[1]):
        continue
      print("Looking at ",current)
      for new in self.get_tile_neighbors2(current[1],came_from):
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        spot = graph[current[1][0]][current[1][1]]
#        print("New: ",new)
        if new is None:
          continue
...
#            continue                 
        if new[0] <= 10 and new[1] <= 14:
          print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
          neighbors +=1
          print("Neighbors scanning: "+str(neighbors)+", Total neighbors: "+str(self.total_walkable))
          new_cost = cost_so_far[current[1]] + 1 #AG: On RedBlob this is graph.cost(current, next) but distance should always be one
          if spot.danger_value is not None:
            new_cost += self.bomb_danger_curve(spot.danger_value)
...
#    print(start)
    ...
    while len(frontier) > 0:
      ...
      if current[1] == destination:
        ...
        break
      #      print("Looking at ",current)
      for new in self.get_tile_neighbors3(current[1]):
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        if new[0] <= 10 and new[1] <= 14:
...
#          print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
    ...
    if len(safety) < 1:
      ...
      print("Player#"+str(self.player.get_number())+" @"+str(start)+", Safety: "+str(destination[1]))
...
#    print("Walkable neighbors: "+str(self.walkable_neighbors)+" and Total walkable: "+str(self.total_walkable))
  ...
  #AG: curve for good idea to put bomb
  def smart_to_bomb_curve(self):
    # y=(x)^{0.5}
...
#      print("Total walkable was 0")
    ...
    y = pow(((5 - self.walkable_neighbors)/self.total_walkable),mod[4])
#    n.write("(walkable_neighbors: "+str(self.walkable_neighbors)+"/ total_walkable: "+str(self.total_walkable)+") to the "+str(mod[4])+" yields: "+str(y)+'\n')
  ...
  #AG: Curve for how much time since haven't moved matters
  def time_impact_curve2(self,ratio):
    #Base curve: y = (1/(1+e^(4x)))+.45
    time_value = 1 - self.normalize_time_value()
    y = 1 / (1 + pow(math.e, mod[5]*time_value))
    y += ratio
  ...
  def play(self):
    #AG: So chosen_movement_action's None can be compared to other action in queue and not cause error
    chosen_movement_action = math.pi
    #AG: Calculate safety for player
    ...
    #AG: Returns reverse probability from normalized output from the curve      
    def use_curve(y):
      #output = 1-y
...
#    self.outputs = []    
    ...
    move_priority = self.bomb_danger_curve(self.game_map.ai_map[current_tile[1]][current_tile[0]].danger_value) 
    ...
    elif self.game_map.tile_has_bomb(current_tile) or self.game_map.ai_map[current_tile[1]][current_tile[0]].danger_value is not None:
...
#      o.write("tile has bomb"+"("+str(escape_priority)+","+str(chosen_movement_action)+")\n")
    ...
    #NH:decisions
    if chosen_movement_action != math.pi or self.move._qsize() < 1:
      ...
      #AG: Since None was getting in here because of the _qsize() added escape direction calculations
      # find maximum
...
#      self.move.put((.95,chosen_movement_action))
#      o.write("Somewhat random movement"+"("+str(.95)+","+str(chosen_movement_action)+")\n")
...
#    else:#if self.game_map.get_map_time() - self.didnt_move_since > 10000:   # didn't move for 10 seconds or more => force move
      ...
      no_move_priority = self.bomb_danger_curve(self.game_map.ai_map[current_tile[1]][current_tile[0]].danger_value)
      ...
      chosen_movement_action = random.choice((PlayerKeyMaps.ACTION_UP,PlayerKeyMaps.ACTION_RIGHT,PlayerKeyMaps.ACTION_DOWN,PlayerKeyMaps.ACTION_LEFT))
#      move_priority = self.game_map.get_map_time() - self.didnt_move_since  #-  use_curve(self.game_map.get_danger_value(current_tile))
      move_priority = self.time_impact_curve()
#      print("Map Time:"+str(self.game_map.get_map_time())+" and Didn't move since:"+str(self.didnt_move_since))
      #self.outputs.append((self.player.get_number(),chosen_movement_action))
...
#    bomb_priority = use_curve(self.game_map.get_danger_value(current_tile)/3000)
    bomb_priority = self.bomb_danger_curve(self.game_map.ai_map[current_tile[1]][current_tile[0]].danger_value) 
    if self.game_map.tile_has_bomb(current_tile):
      # should I throw?
      ...
      '''
        ...
        other_chance = self.smart_to_bomb_curve()
...
#        print("Chance to put bomb now: "+str(chance_to_put_bomb))
    ...
    if self.player.can_box() and not self.player.detonator_is_active():
      if trapped or self.game_map.tile_has_bomb(self.player.get_forward_tile_position()):
        ...
        box_priority = self.bomb_danger_curve(self.game_map.ai_map[current_tile[1]][current_tile[0]].danger_value) - box_choose
        print('in box')
