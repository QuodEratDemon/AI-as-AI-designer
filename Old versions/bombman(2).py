  ...
  def update(self, dt, immortal_player_numbers=[]):
      ...
      for i in range(Map.MAP_WIDTH):
        ...
        for player in self.players:
          ...
          if player_tile is tile and not player.is_dead():
            ...
            self.ai_map[j][i].player = player.get_number()
            isPlayer = True
        if isPlayer is False:
          k2.write(str(tile.kind)+'  ')
          self.ai_map[j][i].player = None
    ...
    for j in range(Map.MAP_HEIGHT):
      ...
      for i in range(Map.MAP_WIDTH):
        ...
        if curr.player is not None and curr.danger_value is None:
          l.write("P"+str(curr.player)+"   ")
        elif curr.danger_value is not None and curr.player is None:
          ...
          if curr.danger_value < 1000:
            ...
            l.write(" ")
        elif curr.danger_value is not None and curr.player is not None:
          l.write("P"+str(curr.player)+"!  ")
...
#          allies += 1
  ...
  def play(self):
    ...
    #Check to see if action needs to be repeated because of special movement
    ...
    #AG: text addition
    strcurr = str(current_tile)
    strtrap = str(trapped)
    stresc = str(escape_direction_ratings)
    h.write(strIdentity + " is at " + strcurr + ". Trapped: "+strtrap+". Best escape: "+stresc+"\n")
    ...
    #NH: kinda hard to explain what im trying to do
    else: #elif use_curve(self.game_map.get_danger_value(current_tile))  :
      ...
      h.write("Movement choice for not on bomb: "+strchosen+". 1 is up, 2 is right, 3 is down, and 4 is left\n")
    ...
    if self.game_map.tile_has_bomb(current_tile):
      ...
      if self.player.can_throw(): #and max(escape_direction_ratings) == 0:
        ...
        bomb_priority = (1 - use_curve(self.game_map.get_danger_value(current_tile))) - 10 
    ...
    if self.player.can_box() and not self.player.detonator_is_active():
      if trapped or self.game_map.tile_has_bomb(self.player.get_forward_tile_position()):
        ...
        box_priority = (1 - use_curve(self.game_map.get_danger_value(current_tile)) ) - box_choose
        print('in box')
