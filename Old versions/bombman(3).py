...
#AG: Declaring base mod list. 0: danger_curve, 1,2,3: bomb_ratio_curve
mod = [1.0001,.3,.2,.3]
...
#        print("Neighbor: ("+str(new[0])+","+str(new[1])+")")
        ...
        if len(spot.player) > 1:
          ...
          if SameAsPlayer is False:
            ...
            playerFound = True
            break
        if new not in visited and self.distance_formula(start,new) <= 2 and new[0] <= 10 and new[1] <= 14:
#          print("Enqueuing ",new,"@ distance ",self.distance_formula(start,new))
          heappush(frontier, (new[0],new[1]))
          visited[new] = current
    ...
    frontier = []
    i = 0
    heappush(frontier, (0, (i, state, "starting")))
    cost_so_far[state] = 0
    Found = False
...
#     print(current_state)     
      if is_goal(current_state):
        ...
        for action, new_state, cost in graph(current_state):
          new_cost = cost_so_far[current_state] + cost
          if new_state not in cost_so_far or new_cost < cost_so_far[new_state]:
            cost_so_far[new_state] = new_cost
#           path_to_here = path_returner((current_state,action),(action,state))
            priority = new_cost + heuristic(new_state, action, current_action) #changed heuristic to new_state
            heappush(frontier, (priority, (i,new_state, action)))
            i+=1
            came_from[(new_state,action)] = (current_state, current_action)
            
    if Found == True:
      print(time() - start_time, 'seconds.')
      print("Final Cost: ",cost_so_far[current_state])
    #Red Blob Code for retrieving path
      current = destination
      plan = [(current, current_action)]
      while current != state:
        current, current_action = came_from[(current, current_action)]
        plan.append((current,current_action))
      plan.reverse()
    '''
  ...
  def rate_tile(self, tile_coordinates):
    ...
    def normalize_bomb_danger(danger):
      newvalue = danger/3000
      return newvalue
                  
    #AG: Utility curve 
    def bomb_danger_curve(danger):
      #AG: Base curve is y=(1/1.001)^x
      y = pow((1/mod[0]),danger)
      return y
      
    if danger != 5000:
      normy_danger = normalize_bomb_danger(danger)
      curve_score = bomb_danger_curve(danger)
      #AG: Output
      str_danger = 'Danger: ' + str(danger)+ ' '
      str_normy_danger = 'Norm: ' + str(normy_danger)+ ' '
      str_curve_score = 'Curve Score: ' + str(curve_score)+ ' '
      g.write(str_danger)
      g.write(str_normy_danger)
      g.write(str_curve_score)
      
    score = bomb_danger_curve(danger)
...
#          allies += 1
  ...
  def play(self):
    ...
    move_priority = 1 - use_curve(self.game_map.get_danger_value(current_tile)) 
    ...
    #NH: kinda hard to explain what im trying to do
      ...
      # find maximum
      best_rating = use_curve(escape_direction_ratings[0])
      best_action = PlayerKeyMaps.ACTION_UP
      best_direction = "up"
      
      if use_curve(escape_direction_ratings[1]) > best_rating:
        best_rating = escape_direction_ratings[1]
        best_action = PlayerKeyMaps.ACTION_RIGHT
        best_direction = "right"
        
      if use_curve(escape_direction_ratings[2]) > best_rating:
        best_rating = escape_direction_ratings[2]
        best_action = PlayerKeyMaps.ACTION_DOWN
        best_direction = "down"
      
      if use_curve(escape_direction_ratings[3]) > best_rating:
        best_rating = escape_direction_ratings[3]
        best_action = PlayerKeyMaps.ACTION_LEFT
        best_direction = "left"
      
      move_priority = 1 - use_curve(self.game_map.get_danger_value(current_tile) - best_rating)
    ...
    if chosen_movement_action != None:
      ...
      no_move_priority = use_curve(self.game_map.get_danger_value(current_tile))
      self.move.put((no_move_priority,None))
    
      #NH: Yet again, need a heuristic for this
      chosen_movement_action = random.choice((PlayerKeyMaps.ACTION_UP,PlayerKeyMaps.ACTION_RIGHT,PlayerKeyMaps.ACTION_DOWN,PlayerKeyMaps.ACTION_LEFT))
      move_priority = use_curve(self.game_map.get_map_time() - self.didnt_move_since)  #-  use_curve(self.game_map.get_danger_value(current_tile)) 
      #self.outputs.append((self.player.get_number(),chosen_movement_action))
      self.move.put((move_priority,chosen_movement_action))
    ...
    bomb_laid = False
    bomb_priority = use_curve(self.game_map.get_danger_value(current_tile)) 
    if self.game_map.tile_has_bomb(current_tile):
      # should I throw?
      ...
      if self.player.can_throw(): #and max(escape_direction_ratings) == 0:
        ...
        bomb_priority = ( use_curve(self.game_map.get_danger_value(current_tile))) - 10 
    ...
    if self.player.can_box() and not self.player.detonator_is_active():
      if trapped or self.game_map.tile_has_bomb(self.player.get_forward_tile_position()):
        ...
        box_priority = (use_curve(self.game_map.get_danger_value(current_tile)) ) - box_choose
        print('in box')
...
#      self.recompute_compute_actions_on = current_time + random.randint(AI.REPEAT_ACTIONS[0],AI.REPEAT_ACTIONS[1])
    ...
    if self.player.detonator_is_active():
       ...
       if random.randint(0,2) == 0 :
        det_priority = use_curve(self.game_map.get_danger_value(current_tile))
        print('in det')
    ...
    chosen_move = self.move.get()
    chosen_action = self.queue.get()
    print(chosen_action)
    self.outputs.append((self.player.get_number(),chosen_move[1]))
    self.outputs.append((self.player.get_number(),chosen_action[1]))
    return self.outputs
